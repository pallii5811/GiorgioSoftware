...
for file in \
  ...
  "src/lib/sanita/crawl-slice-runner.ts"; do
  test -f "$staging/$file"
done
...
cp "$app/src/lib/sanita/crawl-slice-runner.ts" "$backup/src/lib/sanita/"
...
install -m 0644 \
  "$staging/src/lib/sanita/crawl-slice-runner.ts" \
  "$app/src/lib/sanita/crawl-slice-runner.ts"
...
printf 'ARCHIVE_STATE=%s\nBACKUP=%s\nTERRITORY_FAIR_SLICES_DEPLOY_OK\n' \
