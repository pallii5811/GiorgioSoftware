...
const { awards, year, scanned } = await fetchAnacAwards(region, { max: 15 });
console.log(`Anno dataset: ${year ?? "N/A"} | righe regione scansionate: ${scanned}`);
...
for (const a of awards.slice(0, 10)) {
  ...
  console.log(`    CIG=${a.cig} €${Math.round(a.amount).toLocaleString("it-IT")} | ${a.object.slice(0, 70)}…`);
}
