#!/usr/bin/env bash
set -euo pipefail

app="/opt/leadsniper"
stage="/tmp/codex-policy-version-fix"
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
backup="/opt/leadsniper/backups/policy-version-$stamp"
files=(
  "src/lib/sanita/crawler.ts"
  "src/lib/sanita/crawl-slice-runner.ts"
  "src/lib/sanita/frontier-store.ts"
  "src/lib/sanita/lead-crawl-runtime.ts"
  "src/lib/sanita/scan-engine.ts"
  "src/lib/sanita/policy-version-selection.ts"
  "scripts/test-policy-version-selection.mjs"
  "scripts/test-policy-version-frontier.mjs"
)

rollback() {
  for file in "${files[@]}"; do
    if [[ -f "$backup/$file" ]]; then
      install -D -m 0644 "$backup/$file" "$app/$file"
    elif [[ -f "$backup/.absent/${file//\//__}" ]]; then
      rm -f "$app/$file"
    fi
  done
}
trap rollback ERR

mkdir -p "$backup/.absent"
for file in "${files[@]}"; do
  if [[ -f "$app/$file" ]]; then
    mkdir -p "$backup/$(dirname "$file")"
    cp "$app/$file" "$backup/$file"
  else
    touch "$backup/.absent/${file//\//__}"
  fi
  install -D -m 0644 "$stage/$file" "$app/$file"
done

cd "$app"
npx tsx scripts/test-policy-version-selection.mjs
npx tsx scripts/test-policy-version-frontier.mjs

trap - ERR
printf 'BACKUP=%s\n' "$backup"
sha256sum "${files[@]}"
